...
/// @brief  Searches the input string for the first occurring word among those specified as an argument.
...
/// @return The position of the first occurring string. If no matches are found, the function returns string::npos.
size_t find_first_of(const std::string &input, const std::vector<std::string> &find, size_t &index_found, const size_t &pos = 0)
{
...
/// @brief  Removes whitespace from the end of a string and returns a new string, without modifying the original string.
...
/// @brief  Removes whitespace from the beginning of a string and returns a new string, without modifying the original string.
/// @param str Input string.
/// @return New string without leading spaces.
std::string trim_left(const std::string &str)
{
    size_t pos = 0;
    for (; pos < str.size() && std::isspace(str[pos], std::locale::classic()); ++pos) {};
    return str.substr(pos);
}

/// @brief  Removes whitespace from both ends of a string and returns a new string, without modifying the original string.
/// @param str Input string.
...
/// @brief Divides a string into a list of substrings by searching for the separator.
...
std::vector<std::string> split(const std::string &str, const std::string &separator, const size_t offset = 0, const size_t end = std::string::npos)
{
...
/// @brief Divides a string into a list of substrings by searching for the separators.
/// @param str String to divide.
/// @param separators List of strings describing where each split should occur. 
/// @param offset Index of character to search from (default 0).
/// @param end Index of the character to search within.
/// @return A list of strings, split at each point where a separator occurs in the given string.
std::vector<std::string> split(const std::string &str, const std::vector<std::string>& separators, const size_t offset = 0, const size_t end = std::string::npos)
{
    std::vector<std::string> tokens;
...
/// @brief Converts a string to lowercase letters and returns a new string, without modifying the original string.
/// @param str Input string.
...
/// @return True if a string contains a specified string, otherwise it returns false.
bool contains(const std::string& str, const std::string& find, bool case_sensitive = true)
{
    if (case_sensitive)
...
inline bool get_text_between_delimiters(const std::string & input, size_t &start, size_t &end, const std::string &open_delimiter, const std::string &close_delimiter)
{
    ...
    {
        const std::vector<std::string> delimiters = { open_delimiter, close_delimiter };
...
inline std::string get_text_between_delimiters(const std::string &input, const std::string& open_delimiter, const std::string& close_delimiter)
{
    size_t start = 0;
    size_t end = 0;

    if (get_text_between_delimiters(input, start, end, open_delimiter, close_delimiter))
        return input.substr(start + open_delimiter.size(), end - start - open_delimiter.size());
