...
template <concepts::Graph G> void serialize(std::ostream &out, const G &graph) {
    GraphvizProperties empty_map;
    return serialize(out, graph, [&](Vertex) { return empty_map; }, [&](Edge) { return empty_map; });
}
...
GraphvizProperties parse_properties(std::string &attributes_list) {
    ...
    while (utils::get_text_between_delimiters(attributes_body, quote_start, quote_end, "\"", "\"")) {
        quote_values.emplace_back(attributes_body.substr(quote_start + 1, quote_end - quote_start - 1));
        attributes_body.replace(quote_start, quote_end - quote_start + 1, replacement_token);
    }
...
void deserialize(
    ...
    const std::string string_graph((std::istreambuf_iterator<char>(in)),std::istreambuf_iterator<char>());
    ...
    // parse statements
    std::vector<std::string> statements = utils::split(string_graph, STATEMENT_SEPARATORS, body_start + 1, body_end);
            ...
            if (!source_vertex_name.empty()) {
                ...
                for (size_t i = 1; i < edge_vertices.size(); ++i) {
                    ...
                    if (!target_vertex_name.empty()) {
                        ...
                        Edge e = graph.add_edge(Vertex{inserted_vertices[source_vertex_name]}, Vertex{inserted_vertices[target_vertex_name]});
